...
async function fetchHourlyForecastData(city) {
  ...
  try {
    ...
    const hourlyForecastData = data.list.filter(item => item.dt_txt.includes("12:00:00")); // Filter hourly data for a specific time
...
async function createHourlyForecastChart(city) {
  ...
  if (hourlyForecastData) {
    ...
    const hourlyTemperatures = hourlyForecastData.map(hourData => hourData.main.temp);
...
function updateOtherDetails(data, weatherDescription) {
  ...
  otherDetails.innerHTML = `
    <strong>Feels like</strong> ${feelsLike} °C. ${weatherDescription}. ${getWindDescription(windSpeed)}<br><br>
